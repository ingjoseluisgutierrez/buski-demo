...
def actualizar_datos():
    ...
    for tienda, nombre, precio, url in productos:
        cursor.execute("""
            INSERT INTO offers (tienda, nombre_producto, precio, url, fecha_consulta)
            VALUES (?, ?, ?, ?, ?)
        """, (tienda, nombre, precio, url, datetime.now().isoformat()))
...
# Leer base de datos
conn = sqlite3.connect(db_file)
query = "SELECT tienda, nombre_producto, precio, url FROM offers ORDER BY precio ASC"
...
st.dataframe(df_filtrado[["tienda", "nombre_producto", "precio", "url"]], use_container_width=True)
